...
const ScheduleCalendar = () => {
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 p-1">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex bg-slate-800/50 p-1 rounded-xl border border-slate-700/50">
            {["month", "week", "day"].map((item) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all duration-300 ${view === item
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={goToCurrent}
              className="text-xs font-bold px-3 py-1 bg-slate-800 text-indigo-400 rounded-lg border border-slate-700 hover:bg-slate-700 transition-colors"
            >
              TODAY
            </button>
            <button
  ...
  const CustomEvent = ({ event }) => {
    ...
    return (
      <div
        className={`h-full w-full p-2 rounded-lg border-l-4 flex flex-col justify-between transition-all hover:scale-[1.02] cursor-pointer
        ${isTeaching
      ...
      >
        <div>
          <div className="flex items-center justify-between mb-1">
            <span
              className={`text-[10px] font-black uppercase tracking-wider ${isTeaching ? "text-indigo-300" : "text-emerald-300"}`}
            >
            ...
            {event.resource.meetLink && (
              <Video
                size={12}
                className={isTeaching ? "text-indigo-400" : "text-emerald-400"}
              />
            )}
          </div>
