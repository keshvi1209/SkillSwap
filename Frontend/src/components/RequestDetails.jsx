...
const RequestDetails = () => {
  const { id } = useParams(); // Using request ID as the Chat Room ID
  const navigate = useNavigate();
  const location = useLocation();
  const userId = localStorage.getItem("id"); // To identify "My" messages
  ...
  const socketRef = useRef(null); // Persist socket connection
  ...
  useEffect(() => {
    ...
    // 2. Join the specific room (using Request ID)
    ...
    // 4. OPTIONAL: Fetch existing chat history from DB here
  ...
  const handleSendMessage = async () => {
    ...
    const messageData = {
      room: id,
      authorId: userId,
      authorName: "Teacher", // You can fetch actual name from user context
      ...
      <div className="w-full max-w-5xl bg-slate-900 rounded-[2rem] shadow-2xl overflow-hidden border border-slate-800 flex flex-col md:flex-row min-h-[600px] max-h-[90vh] relative">
        ...
        <div className="md:w-1/3 bg-slate-800/40 p-8 flex flex-col justify-between border-r border-slate-800">
          <div className="space-y-8">
            <button onClick={() => navigate(-1)} className="flex items-center text-slate-400 hover:text-indigo-400 transition-colors text-sm group">
            ...
            <div className="flex flex-col items-center md:items-start space-y-4">
              <div className="h-20 w-20 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[1.5rem] flex items-center justify-center shadow-xl ring-4 ring-slate-900/50">
                <span className="text-2xl font-black text-white">{request.studentName?.charAt(0) || "S"}</span>
              </div>
          ...
          <div className="space-y-3 mt-10">
             <button onClick={() => setIsChatOpen(true)} className="w-full flex items-center justify-center space-x-2 py-3 bg-slate-700/40 hover:bg-slate-700 text-white border border-slate-600/50 rounded-xl transition-all font-semibold">
                <MessageSquare size={18} />
                <span>Open Chat</span>
             </button>
             <button onClick={() => navigate("/requests/new")} className="w-full flex items-center justify-center space-x-2 py-3 bg-indigo-600 text-white hover:bg-indigo-500 rounded-xl transition-all font-semibold shadow-lg shadow-indigo-600/10">
                <PlusCircle size={18} />
        ...
        <div className="flex-1 p-8 md:p-12 flex flex-col overflow-y-auto">
          {/* ... (Existing Skill and Slots UI - No changes needed here) ... */}
           <div className="mb-10">
          ...
          <div className="mt-10 pt-8 border-t border-slate-800 grid grid-cols-2 gap-4">
            <button onClick={handleScheduleMeeting} disabled={isScheduling} className="flex items-center justify-center space-x-2 py-4 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-2xl font-bold transition-all border border-slate-700 disabled:opacity-50">
          ...
          <div className="absolute inset-0 z-50 flex justify-end bg-slate-950/60 backdrop-blur-sm transition-all duration-300">
            <div className="w-full md:w-96 bg-slate-900 h-full shadow-2xl flex flex-col border-l border-slate-800 animate-in slide-in-from-right duration-300">
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-900/50">
                {/* Initial Request Message (Static) */}
